...
class RegistrationTestCase(APITestCase):
    ...
    def test_registration(self):
        data = {
            'username': 'testuser',
            'email': 'test@user.com',
            'password1': 'change1234678',
            'password2': 'change1234678'
        }
...
class ProfileViewSetTestCase(APITestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(username='newUser', password='change123456')
    ...
    def test_profile_update_by_owner(self):
        data = {
            'city': 'Madrid', 'bio': 'Text'
        }
        response = self.client.put(reverse('profile-detail', kwargs={'pk': 1}),
                                   data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(json.loads(response.content),
                         {'id': 1, 'city': data['city'], 'bio': data['bio'], 'avatar': None, 'user': self.user.username}
                         )

    def test_profile_update_by_random_user(self):
        random_user = User.objects.create_user(username='randomUser', password='change123456')
...
class ProfileStatusViewSetTestCase(APITestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(username='newUser', password='change123456')
        self.status = ProfileStatus.objects.create(user_profile=self.user.profile, status_content='status texts')
        self.token = Token.objects.create(user=self.user)
        self.api_authentication()
    ...
    def test_status_create(self):
        data = {'status_content': 'Albus zetas ducunt ad gluten.'}
        response = self.client.post(self.list_url, data)

        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.data['user_profile'], 'newUser')
        self.assertEqual(response.data['status_content'], data['status_content'])
    ...
    def test_status_update_owner(self):
        data = {'status_content': 'The scholar visualizes affirmation which is not great.'}
